...
// error_count.count AS error_count
 ...
 //   	public function pj_status_list($id){ // pj_id
	...
	public function import_sentence($pj_id,$title,$sentence,$kind,$scoring,$critique){
		...
		$taste = array("(“)","(”)","(’)","(`)");
		$taste_replace = array('"','"',"'",",");
		$sentence_tags_del = preg_replace($taste, $taste_replace, $sentence_tags_del);

		$title = preg_replace($taste, $taste_replace, $title);
		...
		if($essay_table_insert){
			$sentence = preg_replace($taste, $taste_replace, $sentence);
			$critique = preg_replace($taste, $taste_replace, $critique);
	...
	public function equal_distribute($total_essay_count,$pj_id){		
		...
		if($division > 0){
			...
			foreach ($data->result() as $value) {
				...
				foreach ($essays->result() as $essay) {
					...
					$raw_txt = trim(mysql_real_escape_string($essay->essay)); // orignal 태그 없는 에세이!
					...
					$insert = $this->db->query("INSERT INTO tag_essay(usr_id,essay_id,prompt,raw_txt,kind,type,start_date,pj_id,editing,critique,scoring,tagging,draft,word_count) 
													VALUES('$usr_id','$essay_id','$title','$raw_txt','$kind','$type',now(),'$pj_id','$sen_editing','$critique','$scoring','$data','1','$word_count')");
					if($insert){
						$this->db->query("UPDATE essay SET chk = 'Y' WHERE id = '$essay_id'");						
					}else{
						return '1';
			...
			if($remainder_essay->num_rows() > 0){				
				foreach ($remainder_essay->result() as $value) {
					...
					$remainder_insert = $this->db->query("INSERT INTO tag_essay(usr_id,essay_id,prompt,raw_txt,kind,type,start_date,pj_id,editing,critique,scoring,tagging,draft,word_count) 
															VALUES('$re_usr_id','$re_essay_id','$title','$raw_txt','$kind','$type',now(),'$pj_id','$sen_editing','$critique','$scoring','$data','1','$word_count')");
   			...
   			if($select->num_rows() > 0){
				...
				$sen_editing = mysql_real_escape_string(trim($sen_editing));
   				...
   				$ins = $this->db->query("INSERT INTO tag_essay(usr_id,essay_id,prompt,raw_txt,kind,type,start_date,pj_id,editing,critique,scoring,tagging,draft,word_count) 
   											VALUES('$mem_id','$essay_id','$prompt','$sen','$kind','$type',now(),'$pj_id','$sen_editing','$critique','$scoring','$data','1','$word_count')");
