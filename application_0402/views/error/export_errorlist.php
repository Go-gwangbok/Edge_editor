...
function ajaxPost(url,data){
	$.post(url,data,function(json) {				
		...
		for(var i = 0; i < json['data_list'].length; i++) {
			...
			$('tbody#list').append('<tr id='+i+' class="rowbtn" style="cursor:pointer;" href="/errordata/error_edit/'+data_list[i]['usr_id']+'/'+data_list[i]['essay_id']+'/'+data_list[i]['type']+'/'+pj_id+'"><td class="text-center">'+num+'</td><td>'																				 
					...
					+data_list[i]['type']+'</td><td width="80px;" class="text-center"><button class="btn btn-danger btn-sm">Error edit</button></td>');					
			num++;			
		}								
