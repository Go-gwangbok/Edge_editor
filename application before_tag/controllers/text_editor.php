...
class Text_editor extends CI_Controller {
	...
	function get_datas($cate,$essay_id,$type,$pj_id){
		...
		if($cate == 'draft' || $cate == 'tbd'){
			...
			$datas = $this->all_list->get_completed($essay_id,$type);			
		}
		...
		$editing = str_replace('<p>', '', $datas->editing);
	...
	public function draft_save(){ //<br>을 다시 \r\n으로 // 0
		if($this->session->userdata('is_login')){
			...
			$result = $this->all_list->draft($usr_id,$essay_id,$editing,$critique,$tagging,$type,$score1,$score2,$time);
	...
	public function submit(){
		if($this->session->userdata('is_login')){			
			...
			$result = $this->all_list->submit($usr_id,$essay_id,$editing,$critique,$tagging,$type,$score1,$score2,$time);
	...
	public function editsubmit(){ //0
		if($this->session->userdata('is_login')){			
			...
			$result = $this->all_list->editsubmit($usr_id,$essay_id,$editing,$critique,$tagging,$type,$score1,$score2);
...
/* ======================================================================================================================== 
   ...
   function admin_get_datas($cate,$essay_id,$type){
		if($cate == 'error'){
			$rows = $this->all_list->get_error_Essay($essay_id,$type);
			$data['cate'] = 'musedata';
			$this->load->view('head',$data);
		}elseif($cate == 'tbd' || $cate == 'history' || $cate == 'admin_export'){
		...
		if($rows == false){
			...
			$data['edit_writing'] = preg_replace("/[\n\r]/","<br>",str_replace('"','&quot',$rows->editing));			
	...
	public function essays($cate,$essay_id,$type){
		if($this->session->userdata('is_login')){
			$data = $this->admin_get_datas($cate,$essay_id,$type);		
	...
	public function error($essay_id,$type,$pj_id){
		if($this->session->userdata('is_login')){			
			$data = $this->admin_get_datas('error',$essay_id,$type);		
	...
	public function service_comp($classify,$essay_id,$type,$month,$year){
		if($this->session->userdata('is_login')){						
			...
			$data = $this->admin_get_datas('service',$essay_id,$type);		
			$rows = $this->all_list->get_admin_comp($essay_id,$type);
			...
			// 	$data['title'] = str_replace('"', '&quot', $rows->prompt);
			// 	$data['edit_writing'] = str_replace('"','&quot',$rows->editing);
	...
	public function admin_draft_save(){ //<br>을 다시 \r\n으로 // 0
		if($this->session->userdata('is_login')){			
			...
			$result = $this->all_list->admin_tbd_draft($table_id,$editing,$critique,$tagging,$type,$scoring,$score2,$time);
	...
	public function admin_submit(){
		if($this->session->userdata('is_login')){			
			...
			$result = $this->all_list->admin_tbd_submit($usr_id,$table_id,$editing,$critique,$tagging,$type,$scoring,$score2,$time);
