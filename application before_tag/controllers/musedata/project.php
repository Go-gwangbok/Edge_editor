...
class Project extends CI_Controller {
	...
	public function add_users(){ // 0
		if($this->session->userdata('is_login')){
			...
			$result = $this->all_list->pj_add_users($pj_id,$users);			
			$json['result'] = $result;
		}else{
			redirect('/');
		}		
	...
	public function error(){ //error 신고 입력! DB--> error_essay //0
		if($this->session->userdata('is_login')){			
			...
			$result = $this->all_list->error_proc($essay_id,$usr_id,$type);
			$json['result'] = $result;
		}else{
			redirect('/');
		}
	...
	public function discuss(){ //error 신고 입력! DB--> error_essay //0
		if($this->session->userdata('is_login')){			
			...
			$result = $this->all_list->discuss_proc($essay_id,$usr_id,$type);
			$json['result'] = $result;
		}else{
			redirect('/');
		}
