...
class Service_List extends CI_Model{
	...
	function get_service_export_data($service_id, $year,$month,$limit,$page_list){
		$query = "SELECT service_data.*,usr.name
					FROM service_data
					LEFT join usr ON usr.id = service_data.usr_id
					WHERE service_data.sub_date
					BETWEEN  '".$year."-".$month."-01 00:00:00'
	...
	public function draft_servicedata($essay_id,$editing,$critique,$tagging,$score1,$score2,$time){		
		$confirm = $this->db->query("SELECT * FROM service_data WHERE essay_id = '$essay_id'");
		if($confirm->num_rows() > 0){
			return $this->db->query("UPDATE service_data SET editing = '$editing',critique = '$critique',tagging = '$tagging',draft = 1,scoring = '$score1', score2 = '$score2', time = '$time' WHERE essay_id = '$essay_id'");			
		}else{
			log_message('error', "[debug] draft_servicedata essay_id is not existed : $essay_id");
			return false;
		}		
	}
	...
	***/
	...
	public function insert_file($filename, $title){
   	...
   	public function ins_db_file($filename){   	
   		...
   		$confirm = $this->db->query("SELECT * FROM files WHERE filename = '$filename'");
	...
	public function edi_todo_totalcount($usr_id) {
   	...
   	public function discuss_service_proc($id){   		
		return $this->db->query("UPDATE service_data SET discuss = 'N' WHERE essay_id = '$id'"); 
   	}   	
