			...
			<table class="table edge-table-hover">					
			  	<thead>
					<tr id="head" class="service_headlight">
						...
						<th class="text-center" style="width:140px;">Word Count</th>									
	...
	$.ajax({       
        ...
        success: function(json){                        
            ...
            if(result['status']){            
                ...
                $("tr#new").append('<td><div class="text-center" style="height:50px; margin-top:15px;">'+result['data']['words']+'</div></td>');
                $("tr#new").append('<td><div class="text-center" style="height:50px; margin-top:15px;">'+result['data']['date']+'</div></td>');
...
{		
	...
	$.ajax({			
			...
			if(access['status']){											
			    $('<form action="/service/service_editor/museprep" method="POST"/>')
...
function ajaxPostCom(url,data){
	...
	$.post(url,data,function(json) {				
		...
		for(var i = 0; i < json['list'].length; i++) {
			...
			var tmp_prompt = prompt.split("|||");
			if (tmp_prompt.length > 1) {
				prompt = tmp_prompt[0];
			}
			...
			***/
			var start_date = data_list[i]['start_date'];	
			var sub_date = data_list[i]['sub_date'];

	
			var href = '/museprep/view_essay/';					
			...
			$('tbody#list').append('<tr id='+i+' style="cursor:pointer;" class="clickableRow" href="'+href+essay_id+'/'+'"><td class="text-center">'+num+'</td><td>'
				...
				+start_date+'</td><td class="text-center">'
				+sub_date+'</td><td class="text-center">'				
