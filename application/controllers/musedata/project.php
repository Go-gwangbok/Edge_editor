...
class Project extends CI_Controller {
	...
	{			
		if($this->session->userdata('is_login')){
			...
			if($classify == 0){ // Admin			
				...
				$data['cate'] = 'admin';
				$this->load->view('/musedata_view/admin_index',$data);		
	...
	function get_project(){
		if($this->session->userdata('is_login')){
			...
			$json['data'] = $this->all_list->editor_pjlist($usr_id);		
		}else{
			redirect('/');
		}	
	...
	// public function essay($cate,$pj_id,$usr_id){ // 0
	// 	if($this->session->userdata('is_login')){			
	// 		$data['cate'] = 'musedata';
	// 		$this->load->view('head',$data);			
	...
	public function board($cate,$pj_id,$usr_id){ // 0
		if($this->session->userdata('is_login')){			
			$data['cate'] = 'musedata';
			$this->load->view('head',$data);			
			...
			$name = $this->all_list->usr_pj_name($usr_id,$pj_id);				
			$data['usr_name'] = $name->usr_name; // Admin 에 필요!
			$data['pjName'] = $name->pj_name;
			$data['editor_id'] = $usr_id;
			$data['pj_id'] = $pj_id;
			$data['cate'] = $cate;
			...
			if($this->session->userdata('classify') == 0){ // Admin
				$this->load->view('members_essay_view',$data);					
			}else{ // Editor
				$this->load->view('pjboard',$data);						
			}
	...
	public function page_list(){ //0
		if($this->session->userdata('is_login')){						
			...
			if($cate == 'todo' || $cate == 'draft'){
				...
				$total_count = $this->all_list->pj_comp_totalcount($pj_id,$editor_id); 
				$json['total_count'] = $total_count->count;
				$json['list'] = $this->all_list->admin_pj_done($pj_id,$editor_id,$page,$limit,$list); // history							
			}elseif($cate == 'tbd'){
				$total_count = $this->all_list->discuss_count($pj_id,$editor_id);
				$json['total_count'] = $total_count->count;
				$json['list'] = $this->all_list->discuss_list($pj_id,$editor_id,$limit,$list);
			}elseif($cate == 'history'){
				$total_count = $this->all_list->pj_history_totalcount($pj_id,$editor_id); 
				$json['total_count'] = $total_count->count;				
				$json['list'] = $this->all_list->admin_pj_history($pj_id,$editor_id,$page,$limit,$list); // history							
			}elseif($cate == 'export'){				
				$export_page_count = $this->all_list->export_page_count($pj_id);
				$json['export_page_count'] = $export_page_count->count;
	...
	public function members($id) { // 0	id == pj_id	
		if($this->session->userdata('is_login')){
			$data['cate'] = 'musedata';
			$this->load->view('head',$data);			
					
			$list = $this->all_list->pj_inmembers_list($id);
			$data['memlist'] = $list;
			$data['pjName'] = $list[0]->pj_name;
			$data['kind_id'] = $list[0]->kind_id;
	...
	public function stats($pj_id) { // 0		
		if($this->session->userdata('is_login')){
			$data['cate'] = 'musedata';
			$this->load->view('head',$data);			
					
			$list = $this->all_list->getproject_name($pj_id);			
			$data['pjName'] = $list->name;
			
			$dos_avg = $this->all_list->dos_avg(); 
			$data['total_word_count'] = $dos_avg->total_word_count;
			$data['replace_count'] = $dos_avg->replace_count;

			$stats_count = $this->all_list->pj_totalcount($pj_id); 
			$data['total'] = $stats_count->count;
			$data['draft'] = $stats_count->draft;
			$data['submit'] = $stats_count->submit;
			$data['discuss'] = $stats_count->discuss;
			$data['todo'] = $stats_count->todo;
			$data['pj_total_word_count'] = $stats_count->pj_total_word_count;

			$data['pj_id'] = $pj_id; // project id.
			$data['cate'] = 'stats';
	...
	public function share($pj_id,$usr_id){ // 0
		if($this->session->userdata('is_login')){			
			$data['cate'] = 'musedata';
			$this->load->view('head',$data);			
			...
			$name = $this->all_list->get_user($usr_id,$pj_id);				
			$data['usr_name'] = $name->name;
			$data['editor_id'] = $usr_id;
			$data['pj_id'] = $pj_id;
			$data['cate'] = 'share';
	...
	// public function membertbd($usr_id,$pj_id){ // 0
	// 	if($this->session->userdata('is_login')){		
	// 		$data['cate'] = 'musedata';
	// 		$this->load->view('head',$data);
	...
	public function new_pj(){ //0
		if($this->session->userdata('is_login')){		
			$data['cate'] = 'musedata';
			$this->load->view('head',$data);			

			$data['mem_list'] = $this->all_list->all_usr();
			$data['data_type'] = $this->all_list->get_data_type();
			$this->load->view('new_project_view',$data);		
			$this->load->view('footer');	
		}else{
			redirect('/');
	...
	public function create_pj(){ //0
		...
		$result = $this->all_list->create_pj($name,$disc,$mem_list,$kind);				
		$json['result'] = $result;
		$this->output->set_content_type('application/json')->set_output(json_encode($json));
	}		

	public function del_project(){ //0
		if($this->session->userdata('is_login')){
			$pj_id = $this->input->post('pj_id');			
	...
	public function add_users(){ // 0
		if($this->session->userdata('is_login')){
			$pj_id = $this->input->post('pj_id');
			$users = $this->input->post('users'); // , 로 구분되어서 온다!
			$kind_id = $this->input->post('kind_id');
			
			$result = $this->all_list->pj_add_users($pj_id,$users,$kind_id);			
			$json['result'] = $result;
		}else{
			redirect('/');
		}		
	...
	public function import($id){ // 0 
		if($this->session->userdata('is_login')){		
			...
			$data['list'] = $this->all_list->new_essayList($id);
			$data['count'] = $this->all_list->import_count($id);
			$pj = $this->all_list->pj_name($id);
	...
	public function errorlist($pj_id){ //0
		if($this->session->userdata('is_login')){			
			$cate['cate'] = 'musedata';
			$this->load->view('head',$cate);			
			$page = 1;
			...
			$this->load->view('/error/project_errorlist',$data);					
			$this->load->view('footer');					
		}else{
			redirect('/');
		}
	...
	public function export($pj_id){ // 0
		if($this->session->userdata('is_login')){			
			$cate['cate'] = 'musedata';
			$this->load->view('head',$cate);			
			$page = 1;
			...
			$name = $this->all_list->export_index($pj_id);				
			$data['pjName'] = $name->name;
			$data['total_count'] = $name->total_count;
			$data['export_count'] = $name->export_count;			
			$data['pj_id'] = $pj_id;
			$data['cate'] = 'export';						
	...
	// public function essay_chk($option,$essay_id){ //0 member_list
	// 	if($this->session->userdata('is_login')){			
	// 		$pj_id = $this->input->post('pj_id');			
	// 		$err_id = array();			
	// 		$done_data = array();		
	...
	function getarraybetweenmod($original_text, $needle1, $needle2) { //0
		$text_array = explode($needle1, $original_text);
		for($i=1; $i<sizeof($text_array); $i++) {
	...
	// public function memchkExport(){ //0
	...
	public function word_count(){ // 0
		$result = $this->all_list->word_count_update();
		$json['result'] = $result;
		$this->output->set_content_type('application/json')->set_output(json_encode($json));		
	}

	public function detecting_count(){ // 0
		$result = $this->all_list->detecting_count();
		$json['result'] = $result;
		$this->output->set_content_type('application/json')->set_output(json_encode($json));
	}
