...
class Text_editor extends CI_Controller {
	...
	public function completed($essay_id,$type,$pj_id){
		if($this->session->userdata('is_login')){
			...
			$rows = $this->all_list->get_completed($usr_id,$essay_id,$type);			
			$pjname = $this->all_list->getproject_name($pj_id);				
	...
	public function submit(){
		if($this->session->userdata('is_login')){			
			...
			$json['word_count'] = $word_count = str_word_count($editing);
			...
			if($result){		
				...
				if(!$error_chk){ // false 라면 DB error_count 에 1을 증가 시킨다.
...
/* ======================================================================================================================== 
   Admin	*/ 
	...
	public function service_comp($essay_id,$type,$month,$year){
		if($this->session->userdata('is_login')){				
			$head['cate'] = 'service';		
			$this->load->view('head',$head);							
