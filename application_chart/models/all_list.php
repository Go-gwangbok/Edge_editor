...
class All_list extends CI_Model{
	...
	function pj_totalcount($pj_id){		
		return $this->db->query("SELECT COUNT( * ) AS count, COUNT( IF( draft =1
									AND discuss =  'Y'
									AND submit =0, 1, NULL ) ) AS draft, COUNT( IF( submit =1, 1, NULL ) ) AS submit, COUNT( IF( discuss =  'Y', NULL , 1 ) ) AS discuss, COUNT( IF( draft =0
									AND submit =0, 1, NULL ) ) AS todo
									FROM tag_essay
									WHERE pj_id =  '$pj_id'
									AND essay_id !=0
									AND active =0")->row();
	...
	// 	$query = "SELECT COUNT( IF( discuss =  'N'
	// 				AND submit =0, 1, NULL ) ) AS tbd, usr.name, SUM( IF( submit =1, TIME, 0 ) ) AS total_time, COUNT( IF( discuss =  'Y', 1, NULL ) ) AS total
	// 				FROM tag_essay
	// 				LEFT JOIN usr ON usr.id = tag_essay.usr_id
	// 				WHERE tag_essay.pj_id = '$pj_id'
	// 				AND tag_essay.active =0
	...
	function stats_data($pj_id){
		$query = "SELECT COUNT( IF( discuss =  'N' AND submit =0, 1, NULL ) ) AS tbd, usr.name, 
					...
					COUNT( IF( discuss =  'Y', 1, NULL ) ) AS total, 
	...
	public function submit($usr_id,$essay_id,$editing,$critique,$tagging,$type,$scoring,$time){
		$confirm = $this->db->query("SELECT * FROM tag_essay WHERE essay_id = '$essay_id' and type = '$type'");
		
		if($confirm->num_rows() > 0){	
			$row = $confirm->row();					
			$pj_id = $row->pj_id;
			$query = $this->db->query("UPDATE tag_essay SET editing = '$editing',critique = '$critique',tagging = '$tagging',scoring = '$scoring',draft = 1,submit = 1,sub_date = now(), time = '$time', discuss = 'Y', usr_id = '$usr_id' WHERE essay_id = '$essay_id' and type = '$type'");
			if($query){				
				return true;
			}else{
				return false;
 ...
 //   	public function pj_status_list($id){ // pj_id
	...
	public function import_mem_sentence($mem_id,$sentence_num,$pj_id){
   	...
   	public function ex_editing_update_service($essay_id,$data,$type){
   		$result = $this->db->query("UPDATE tag_essay SET ex_editing = '$data' WHERE essay_id = '$essay_id' and type = '$type'");
   		return $result;      
   	}
   	...
   	public function editing_update($id,$data){
   		$result = $this->db->query("UPDATE tag_essay SET editing = '$data' WHERE essay_id = '$id' and type = 'musedata'");
   		return $result;      
   	}
