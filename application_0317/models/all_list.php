...
class All_list extends CI_Model{
	...
	function pj_name($id){
   	...
   	function pj_inmembers_list($id){ // pj_id
		return $this->db->query("SELECT usr.id as usr_id, usr.name,count(DISTINCT tag_essay.id) as count, cou.update_count, cou.done_count, project.name as pj_name, SUM( tag_essay.discuss =  'N' ) AS tbd,usr.date
									FROM tag_essay
									...
									WHERE cou.pj_id = '$id' and tag_essay.pj_id = '$id' and tag_essay.essay_id != 0 and tag_essay.active = 0 and tag_essay.pj_active = 0 and usr.classify = 1 and usr.conf = 0 GROUP BY tag_essay.usr_id")->result();
	}
