...
class All_list extends CI_Model{
	...
	public function admin_submit($usr_id,$essay_id,$editing,$critique,$tagging,$type,$scoring,$time){
		$confirm = $this->db->query("SELECT * FROM tag_essay WHERE essay_id = '$essay_id' and type = '$type' and submit = 0");
	...
	public function import_mem_sentence($mem_id,$sentence_num,$pj_id){
   	...
   	public function memchkExportget_essay($essay_id){
   		return $this->db->query("SELECT * FROM tag_essay WHERE essay_id in($essay_id)")->result();
   	}

   	public function ex_editing_update($id,$data){
   		$result = $this->db->query("UPDATE tag_essay SET ex_editing = '$data' WHERE essay_id = '$id'");
   		return $result;      
   	}
