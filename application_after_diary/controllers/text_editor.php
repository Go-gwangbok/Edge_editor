...
class Text_editor extends CI_Controller {
	...
	public function tbd($essay_id,$type,$pj_id){
		if($this->session->userdata('is_login')){
			...
			$pattern = array('({)','(})','(")');
            $replace = array('','','');
            $scoring = preg_replace($pattern, $replace, $scoring);
			...
			$data['edit_writing'] = str_replace('"','&quot',$datas->editing);
			$data['raw_writing'] = $datas->raw_txt;			

			$raw_sen = $datas->raw_txt;
			$convert = str_replace('"', '&quot',$raw_sen); 
			$data['re_raw_writing'] = preg_replace("/[\n\r]/","<br>", $convert);
			$data['discuss'] = $datas->discuss;
			$data['writing'] = '';
			$data['id'] = $datas->id;
			...
			if($kind == 'essay'){
				...
				$this->load->view('/editor/editor_diary',$data);
			}			
			$this->load->view('footer');					
		}else{
			redirect('/');
		...
		if($this->session->userdata('is_login')){				
			...
			if($rows == false){
				...
				if($kind == 'essay'){
					...
					$this->load->view('/editor/admin_diary',$data);
				}
