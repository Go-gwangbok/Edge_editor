	...
	{		
		...
		else
		{
			...
			if($conf){
				...
				$kind = trim(mysql_real_escape_string(substr($classify[0],0,-1)));

				if($kind == 'essay'){ // 종류가 에세이 일경우!
					...
					for($i = 1; $i < $data_cou; $i++){									
						...
						//essay 테이블에 문장 입력!
						if($result){
							$result = $this->all_list->import_sentence($pj_id,$sentence,$structure,$kind,$scoring,$critique);
						}else{
							$data['error'] = $result;		
						}			
					} // For end.					
					
				}elseif($kind == 'diary'){ // 다이어리 일 경우.
					...
					for($i = 1; $i < $data_cou; $i++){									
						...
						$json['EV'] = trim(substr($text[1],1,1));
						$json['TR'] = trim(substr($text[2],1,1));
						$json['SR'] = trim(substr($text[3],1,1));
						$json['Co'] = trim(substr($text[4],1,1));						
						$scoring = json_encode($json);	
						...
						// Diary DB에 저장!
						if($result){
							$result = $this->all_list->import_sentence($pj_id,$diary,$diary,$kind,$scoring,$critique);
						}else{
							$data['error'] = $result;		
						}			
					} // For end.					
