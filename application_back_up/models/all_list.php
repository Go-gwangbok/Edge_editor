...
class All_list extends CI_Model{
	...
	function get_admin_comp($essay_id,$type){
		return $this->db->query("SELECT tag_essay.essay_id as id,tag_essay.* FROM tag_essay WHERE essay_id = '$essay_id' AND type = '$type'")->row();
	}
	...
	function pj_name($id){
   	...
   	function pj_inmembers_list($id){ // pj_id
		return $this->db->query("SELECT usr.id as usr_id, usr.name,sum(tag_essay.essay_id != 0 and tag_essay.type != 'project' ) as count, cou.done_count, project.name as pj_name, SUM( tag_essay.discuss =  'N' ) AS tbd,usr.date,SUM( if(tag_essay.type != 'project' and tag_essay.submit =  0 and tag_essay.discuss = 'Y',1,0)  ) AS share
									FROM tag_essay
	...
	public function get_completed($usr_id,$essay_id,$type){		
		return $this->db->query("SELECT tag_essay.essay_id as id,prompt,raw_txt,editing,tagging,critique,type,scoring,time,discuss,error_essay.active as chk,tag_essay.submit,tag_essay.draft
									FROM tag_essay 
									LEFT JOIN error_essay ON error_essay.essay_id = tag_essay.essay_id									
	...
	public function new_editordel($id){
   	...
   	public function pjlist(){
   		return $this->db->query("SELECT project. * , project.id AS pj_id, project.active AS tbd, COUNT( tag_essay.essay_id ) AS total_count
									FROM project
	...
	public function import_mem_sentence($mem_id,$sentence_num,$pj_id){
   	...
   	public function editor_pjlist($usr_id){
  		// return $this->db->query("SELECT DISTINCT pj_id, name, disc, add_date, project.id
				...
				// 					AND tag_essay.active = 0
				// 					ORDER BY add_date DESC")->result(); 		
  		return $this->db->query("SELECT pj_id, project.name, project.disc, project.add_date, tag_essay.usr_id,count(distinct tag_essay.essay_id) as total_count
									FROM tag_essay
									LEFT JOIN project ON project.id = tag_essay.pj_id
									WHERE usr_id =  '$usr_id'
									AND tag_essay.essay_id != 0
   	...
   	public function export_count($pj_id){
   		return $this->db->query("SELECT count(id) as count FROM tag_essay WHERE pj_id = '$pj_id' and essay_id != 0 and discuss = 'Y' and pj_active = 0 and active = 0 and submit = 1 and ex_editing != ''")->row();
   	}   	

   	public function exporttotal_count($pj_id){
   	...
   	public function memchkExportget_essay($essay_id){
   		return $this->db->query("SELECT * FROM tag_essay WHERE essay_id in($essay_id) and type = 'Tagging'")->result();
   	}

   	public function ex_editing_update($id,$data){
   		$result = $this->db->query("UPDATE tag_essay SET ex_editing = '$data' WHERE essay_id = '$id' and type = 'Tagging'");
   		return $result;      
   	}
   	...
   	public function editing_update($id,$data){
   		$result = $this->db->query("UPDATE tag_essay SET editing = '$data' WHERE essay_id = '$id' and type = 'Tagging'");
   		return $result;      
   	}
