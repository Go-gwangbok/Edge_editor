...
function ajaxPost(url,data){
	$.post(url,data,function(json) {				
		...
		$('div#error').append('<button class="btn btn-default pull-right" disabled><font color="red"><b> Total : '+json['data_count']+'</b></font></button>');
		...
		for(var i = 0; i < json['data_list'].length; i++) {
			...
			$('tbody#list').append('<tr id='+i+' class="rowbtn" href="/error/error_edit/'+data_list[i]['usr_id']+'/'+data_list[i]['essay_id']+'/'+data_list[i]['type']+'/'+pj_id+'"><td class="text-center">'+num+'</td><td>'																				 
