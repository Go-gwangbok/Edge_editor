<div class="container" style="margin-top:-15px;">		
	<div class="row">		
	    ...
	    <label class="pull-right" style="margin-right:10px;" id="total">Total : <?=$total_count;?>&nbsp;&nbsp;</label>				
	    <h3 class="text-center"><?=$pjName;?></h3>	    
	    <!-- Button trigger modal -->	    
	    <div id="error">	 
	    	<a href="/error/export_error/<?=$pj_id;?>"><button class="btn btn-danger pull-left" id="errorbtn" style="margin-top:-10px; margin-left:15px;">Error List<span class="badge" style="background-color:transparent;"><?=$total_count-$export_count;?></span></button></a>
		...
		if(cate == 'error_export'){
			...
			for(var i = 0; i < json['history_list'].length; i++) {
				...
				$('tbody#list').append('<tr class="com" id='+i+' href="/text_editor/comp/'+history_list[i]['essay_id']+'/'+task+'"><td>'+num+'</td><td>'																				 
						...
						+history_list[i]['type']+'</td><td>'
						+timer+'</td>');						
				$('tbody#list tr#'+i).last().append('<td><font class="comp">Completed</font></td>');								
				num++;				
				$('div#pageblock').children().remove();
			
				pageBlock(page,list,export_page_count); // page button.
			}
		}
	});
}
