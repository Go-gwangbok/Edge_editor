...
function ajaxPost(url,data){
	$.post(url,data,function(json) {		
		...
		for(var i = 0; i < json['data_list'].length; i++) {
			...
			$('tbody#list').append('<tr id='+i+' class="hrefclick" href="/text_editor/error/'+essay_id+'/'+task+'/'+pj_id+'"><td class="text-center">'+num+'</td><td>'
				+data_list[i]['prompt'].replace(/"/gi,'')+'</td><td>'
				+data_list[i]['usr_name']+'</td><td>'
				+date+'</td><td>'
				+data_list[i]['type']+'</td>');

			$('tbody#list tr#'+i).last().append('<td class="text-center"><font color="red">&nbsp;&nbsp;&nbsp;Error&nbsp;&nbsp;&nbsp;</font></td>');
			num++;
		}						
