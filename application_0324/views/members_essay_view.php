	...
	$.post(url,data,function(json) {				
		...
		if(cate == 'tbd'){ // T.B.D
			for(var i = 0; i < json['list'].length; i++) {
				...
				$('tbody#list').append('<tr id='+i+' class="clickableRow" href="/text_editor/essays/tbd/'+essay_id+'/'+task+'/'+pj_id+'"><td class="text-center">'+num+'</td><td>'																				 
					...
					+data_list[i]['type']+'</td><td class="text-center"><font class="akacolor"><b>T.B.D</b></font></td>');				
				num++;			
			}	
			...
			for(var i = 0; i < json['list'].length; i++) {
				...
				$('tbody#list').append('<tr id='+i+' class="clickableRow" href="/text_editor/essays/history/'+essay_id+'/'+task+'/'+pj_id+'"><td class="text-center">'+num+'</td><td>'																				 
					+data_list[i]['prompt'].replace(/"/gi,'')+'</td><td class="text-center">'
					+data_list[i]['kind']+'</td><td class="text-center">'
					+date+'</td><td class="text-center">'
					+data_list[i]['type']+'</td>');						
				...
				if(data_list[i]['draft'] == 0){
					$('tr#'+i).append('<td class="text-center"><font class="todo"><b>New</b></font></td>');	
				}else if(data_list[i]['draft'] == 1 && data_list[i]['submit'] == 0){
					$('tr#'+i).append('<td class="text-center"><font class="todo"><b>Edit</b></font></td>');	
				}else if(data_list[i]['draft'] == 1 && data_list[i]['submit'] == 1){
					$('tr#'+i).append('<td class="text-center"><font class="comp"><b>Completed</b></font></td>');	
				}				
				num++;			
			}	
		}			
